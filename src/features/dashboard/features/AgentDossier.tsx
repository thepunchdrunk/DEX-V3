...
const AgentDossier: React.FC<AgentDossierProps> = ({ user }) => {
    ...
    return (
        <div className="space-y-8 animate-fade-in text-neutral-900">
            {/* Header: Identity Card (Premium Glass) */}
            <div className="relative glass-panel rounded-[40px] p-8 md:p-12 overflow-hidden shadow-[var(--shadow-float)] border-white/40">
                {/* Background Decor */}
                <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-gradient-to-br from-brand-red/10 to-transparent rounded-full blur-[100px] -translate-y-1/2 translate-x-1/4 pointer-events-none" />

                <div className="relative z-10 flex flex-col md:flex-row items-center gap-10">
                    {/* Avatar & Rank */}
                    <div className="shrink-0 flex flex-col items-center">
                        <div className="relative group perspective-1000">
                            <div className="w-32 h-32 rounded-[32px] bg-white/20 backdrop-blur-md flex items-center justify-center text-5xl font-black shadow-2xl transition-transform duration-500 group-hover:rotate-y-12 border-4 border-white">
                                {user.name.split(' ').map(n => n[0]).join('')}
                            </div>
                            <div className={`absolute -bottom-4 -right-4 w-12 h-12 rounded-xl ${currentTier.bgColor} ${currentTier.borderColor} border-2 flex items-center justify-center shadow-lg transform rotate-12 transition-transform group-hover:rotate-0`}>
                                <span className={currentTier.color}>{currentTier.icon}</span>
                            </div>
                        </div>
                        <div className={`mt-6 px-4 py-1.5 rounded-full border bg-white/50 backdrop-blur-sm border-neutral-200 text-xs font-black uppercase tracking-widest ${currentTier.color}`}>
                    ...
                    <div className="flex-1 text-center md:text-left">
                        <div className="flex flex-col md:flex-row items-center gap-4 mb-2">
                            <h2 className="text-4xl md:text-5xl font-black tracking-tighter text-neutral-900 drop-shadow-sm">{user.name}</h2>
                            <span className="hidden md:block w-px h-8 bg-neutral-300" />
                            <p className="text-xs font-bold text-neutral-500 uppercase tracking-widest">{user.jobTitle} • {user.department}</p>
                        </div>

                        <p className="text-neutral-600 max-w-xl leading-relaxed mb-8 font-medium">
                        ...
                        <div className="flex flex-wrap items-center justify-center md:justify-start gap-4">
                            <div className="flex items-center gap-3 px-5 py-3 glass-panel rounded-2xl bg-white/40 hover:bg-white/60 transition-colors">
                    ...
                    <div className="flex flex-col gap-3 shrink-0">
                        <button className="p-4 rounded-2xl bg-white/60 border border-white hover:bg-white hover:shadow-lg hover:scale-105 transition-all group">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div className="space-y-6">
                    <section className="glass-panel rounded-[32px] p-8 relative overflow-hidden group hover:shadow-[var(--shadow-float)] transition-all">
                        ...
                        <div className="space-y-4">
                            <div className="p-4 rounded-2xl bg-neutral-50/50 border border-neutral-100 group-hover:bg-white transition-colors">
                                <p className="text-[10px] font-bold text-neutral-400 uppercase tracking-wider mb-1">Institutional ID</p>
                                <p className="text-lg font-mono font-bold text-neutral-900">{user.employeeId}</p>
                            </div>
                            <div className="p-4 rounded-2xl bg-neutral-50/50 border border-neutral-100 group-hover:bg-white transition-colors">
                                <p className="text-[10px] font-bold text-neutral-400 uppercase tracking-wider mb-1">Role Category</p>
                                <span className="px-2 py-1 bg-neutral-200 rounded text-xs font-bold text-neutral-700">{user.roleCategory || 'DESK'}</span>
                            </div>
                            <div className="p-4 rounded-2xl bg-neutral-50/50 border border-neutral-100 group-hover:bg-white transition-colors">
                ...
                <div className="space-y-6">
                    <section className="glass-panel rounded-[32px] p-8 relative overflow-hidden hover:shadow-[var(--shadow-float)] transition-all">
                        ...
                        <div className="aspect-square rounded-2xl bg-neutral-50/30 border border-neutral-100 p-4 relative overflow-hidden mb-6 flex items-center justify-center">
                            ...
                            {MOCK_SKILL_TREE.slice(0, 3).map((branch) => (
                                <div key={branch.id} className="flex items-center justify-between p-3 rounded-xl hover:bg-neutral-50 transition-colors cursor-pointer group">
                                    <div className="flex items-center gap-3">
                                        <div className="w-8 h-8 rounded-lg bg-neutral-100 group-hover:bg-white group-hover:shadow-sm flex items-center justify-center transition-all">
                ...
                <div className="space-y-6">
                    <section className="glass-panel rounded-[32px] p-8 hover:shadow-[var(--shadow-float)] transition-all">
                        ...
                        <div className="grid grid-cols-4 gap-3">
                            {unlockedBadges.slice(0, 8).map(badge => (
                                <div key={badge.id} className="aspect-square rounded-2xl bg-neutral-50/50 border border-neutral-100 flex items-center justify-center text-2xl shadow-sm hover:scale-110 hover:bg-white hover:shadow-md transition-all cursor-help" title={badge.name}>
                                    {badge.icon}
                                </div>
                            ))}
                        </div>
                    ...
                    <section className="glass-panel rounded-[32px] p-8 hover:shadow-[var(--shadow-float)] transition-all">
                        ...
                        <div className="p-6 rounded-3xl bg-neutral-50/50 border border-neutral-100 relative overflow-hidden group hover:bg-white transition-colors">
                    ...
                    <div className="pt-4">
                        <button
                            ...
                            className="w-full py-4 text-xs font-bold text-neutral-400 hover:text-brand-red hover:bg-red-50 rounded-xl uppercase tracking-widest transition-all flex items-center justify-center gap-2 group"
                        >
